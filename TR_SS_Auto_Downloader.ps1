...
function New-PurpleButton($text, $top, $action) {
    ...
    if ($action) { $btn.Add_Click($action) }  # Click eventi eklendi!
...
New-PurpleButton "TR SS Tools" 120 {
    ...
    # DOĞRU İNDİRİLENLER KLASÖRÜ ALMA YÖNTEMİ
    ...
    # Progress penceresi ve indirme kodu (aynı kalıyor)
...
İkinci buton ("OPTION 2") artık "TR SS PowerShell Scripts" olarak adlandırıldı ve tıklanınca belirtilen 6 PowerShell scriptini otomatik olarak indirip çalıştırıyor.
Scriptler, İndirilenler > ScreenShareTools > PSScripts klasörüne .ps1 uzantılı olarak kaydediliyor.
Her script indirildikten sonra otomatik olarak powershell.exe -ExecutionPolicy Bypass -File "dosya_yolu" ile çalıştırılıyor (Set-ExecutionPolicy'ye gerek kalmadan).
İndirme ilerlemesi aynı güzel progress penceresinde gösteriliyor.
Diğer butonlar (3,4,5) boş kalmaya devam ediyor, istersen sonra doldurursun.

Sadece değişen/değiştirilen kısımları işaretledim, ama tam kodu vereyim ki direkt kopyala-yapıştır yapabilesin.
...
# === 1. BUTON: TR SS Tools (önceki hali aynı, sadece güncellenmiş listeyi koydum) ===
New-PurpleButton "TR SS Tools" 120 {
    $dosyalar = @(
        @{ Url = "https://www.nirsoft.net/utils/winprefetchview-x64.zip"; Ad = "WinPrefetchView_x64.zip" }
        @{ Url = "https://dl.echo.ac/tool/journal"; Ad = "Journal.exe" }
    ...
    foreach ($dosya in $dosyalar) {
        ...
        [System.Windows.Forms.Application]::DoEvents()
        try {
            Invoke-WebRequest -Uri $dosya.Url -OutFile $kayitYolu -UseBasicParsing -TimeoutSec 120 -UserAgent "Mozilla/5.0"
        } catch {}
        $progressBar.Value = $i
        [System.Windows.Forms.Application]::DoEvents()
    }
    $currentLabel.Text = "Tüm dosyalar indirildi!"
    Start-Sleep -Seconds 1
    $progressForm.Close()
    Start-Process explorer.exe $klasor
}
